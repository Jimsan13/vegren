...
<style>
    ...
    .form-header {
        ...
        font-size: 1.6rem;
        font-weight: bold;
        color: #2c2c2cff; /* Verde, para utilidades */
        gap: 10px;
...
<div class="form-wrapper">
    <div class="form-card">
        <div class="form-header">
            <i class="fas fa-chart-line"></i> Agregar Utilidad
        </div>
        ...
        <form method="POST" action="{{ route('finanzas.utilidades.store') }}">
            @csrf
            ...
            <input type="text" name="concepto" id="concepto" placeholder="Ej: Venta de Producto X" value="{{ old('concepto') }}" required>

            <label for="categoria"><i class="fas fa-tags"></i> Categoría</label>
            <select name="categoria" id="categoria" required>
                <option value="" disabled {{ old('categoria') ? '' : 'selected' }}>Seleccione categoría</option>
                <option value="Ventas" {{ old('categoria') == 'Ventas' ? 'selected' : '' }}>Ventas</option>
                <option value="Gastos Operativos" {{ old('categoria') == 'Gastos Operativos' ? 'selected' : '' }}>Gastos Operativos</option>
            ...
            <div class="form-actions">
                <a href="{{ route('finanzas.utilidades') }}" class="btn-cancel">
                    <i class="fas fa-arrow-left"></i> Cancelar
                </a>
                <button type="submit" class="btn-submit">
                    <i class="fas fa-check-circle"></i> Guardar Utilidad
                </button>
            </div>
        </form>
    </div>
